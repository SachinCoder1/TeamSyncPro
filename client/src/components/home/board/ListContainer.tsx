...
//   })
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    if (type === 'card') {
      ...
      const destinationList = newOrderedData.find((list) => list._id === destination.droppableId)
      ...
      if (source.droppableId === destination.droppableId) {
        const reorderedCards = reorder(sourceList.tasks, source.index, destination.index)
        ...
        const beforeTaskId = destination.index > 0 ? sourceList.tasks[destination.index - 1]._id : null;
        const afterTaskId = destination.index < sourceList.tasks.length - 1 ? sourceList.tasks[destination.index + 1]._id : null;
        const currentTaskId = sourceList.tasks[destination.index]._id;
        setLoading(true);
        const isReordered = await reorderTaskInBetween(currentTaskId, beforeTaskId, afterTaskId)
        ...
        const beforeTaskId = destination.index > 0 ? destinationList.tasks[destination.index - 1]: null;
        console.log("beforeTaskId: ", beforeTaskId);
        const afterTaskId = destination.index < destinationList.tasks.length - 1 ? destinationList.tasks[destination.index + 1] : null;
        console.log("afterTaskId: ", afterTaskId);
